  ...
  return (
    ...
    >
      <CollapsibleTrigger className="relative flex flex-row items-center w-full group text-start px-3 py-2 not-prose hover:bg-fd-accent">
        ...
        <ChevronDown className="absolute inset-e-2 size-4 text-fd-muted-foreground transition-transform group-data-[state=open]:rotate-180" />
      </CollapsibleTrigger>
      <CollapsibleContent>
        <div className="grid grid-cols-[1fr_3fr] gap-y-4 text-sm p-3 overflow-auto fd-scroll-container border-t">
          ...
          { typeDescriptionLink && (
            <>
              <p className={cn(fieldVariants())}>More Info</p>
              <Link href={typeDescriptionLink} className="underline flex items-center gap-1">
                View Documentation <ExternalLinkIcon size={14} />
              </Link>
            </>
          )}
          {returns && (

...
import { DiscordIcon } from '@/lib/layout.shared';
...
export function resolveLinkItems({
  ...
  githubUrl,
  discordUrl,
}: Pick<BaseLayoutProps, 'links' | 'githubUrl' | 'discordUrl'>): LinkItemType[] {
  const result = [...links];
...
export function useLinkItems({ githubUrl, discordUrl, links }: Pick<BaseLayoutProps, 'links' | 'githubUrl' | 'discordUrl'>) {
  return useMemo(() => {
    const all = resolveLinkItems({ links, githubUrl, discordUrl });

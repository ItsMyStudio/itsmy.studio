...
- the new `LeaderboardEntry[]` return shape
- how `formatValue(entry)` is used
- how `additionalData` can support richer formatting
- how leaderboard values are now separated from the core row layout
...
8. `src/addons/InviteTracker/leaderboards/invites.ts`

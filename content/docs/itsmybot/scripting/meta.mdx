...
<TypeTable
  type={{
    ...
    leaderboard: {
      description: "Optional leaderboard configuration. Only works when `type` is `number` and `mode` is `user`.",
      type: 'object',
      required: false
    }
  }}
...
#### Leaderboard
Number metas in `user` mode can now register a leaderboard automatically.
The `leaderboard` section supports:

<TypeTable
  type={{
    enabled: {
      description: "Enable automatic leaderboard registration for this meta.",
      type: 'boolean',
      required: true
    },
    name: {
      description: "Leaderboard identifier used under `/leaderboard <name>`.",
      type: 'string',
      required: true
    },
    description: {
    ...
    format: {
      description: "Optional value format used instead of the raw number. Supports variables like `[[value]]`.",
      type: 'string',
      required: false
    }
  }}
...
```yaml title='Level Meta with Leaderboard'
metas:
  - key: 'experience'
    type: 'number'
    mode: 'user'
